    ...
    try {
      if (streamCallback) {
        ...
        for await (const event of agent.stream(prompt, runOptions)) {
          // Wrap callback so observer errors never break task execution,
          // mirroring the emitTrace safety pattern.
          try { streamCallback(event) } catch { /* ignore observer errors */ }
